...
def delete_task_by_task_name(task_name):
    ...
    # SQL query to delete task by name
...
class TaskWizardApp:
        ...
        def check_credentials(e):
            ...
            try:
                ...
                if user_data:
                    ...
                    self.page.snack_bar = SnackBar(content=Text("Invalid email or security answer!", color="White"))
                    self.page.snack_bar.open = True
                    self.page.update()
    ...
    def show_registration_page(self):
        ...
        if not userid or not fullname_field or not username_field or not email_field or not password_field or not ask_field:
            self.page.snack_bar = SnackBar(content=Text("Fill all fields!", color="White"))
            self.page.snack_bar.open = True
            self.page.update()
            return  # Exit the function if val
    ...
    #         if not userid or not full_name or not user_name or not email or not password or not security_qna:
    #             self.page.snack_bar = SnackBar(content=Text("Fill all fields!", color="White"))
    #             self.page.snack_bar.open = True
    #             self.page.update()
    ...
    #         c.execute("SELECT id FROM registration WHERE id = ?", (userid,))
    #         if c.fetchone() is not None:
    #             # Create the SnackBar
    #             snackbar = SnackBar(
    #                 content=Text("Registration successful!", color=PRIMARY_COLOR),
    #                 bgcolor=TEXT_COLOR
    #             )
    #             # Add the SnackBar to the page and open it
    #             self.page.snack_bar = snackbar
    #             self.page.snack_bar.open = True
    #             # Update the page to display the SnackBar
    #             self.page.update()
    ...
    #     # Create the SnackBar
    #         snackbar = SnackBar(
    #             content=Text("Error: Username or Email already exist", color=PRIMARY_COLOR),
    #             bgcolor=TEXT_COLOR
    #         )
    #         # Add the SnackBar to the page and open it
    #         self.page.snack_bar = snackbar
    ...
    def handle_registration(self, userid, full_name, user_name, email, password, security_qna):
        try:
            ...
            if not userid or not full_name or not user_name or not email or not password or not security_qna:
                self.page.snack_bar = SnackBar(content=Text("Fill all fields!", color="White"), bgcolor="red")
                self.page.snack_bar.open = True
                self.page.update()
            ...
            c.execute("SELECT id FROM registration WHERE id = ?", (userid,))
            if c.fetchone() is not None:
                # If user ID already exists, show an error message
                snackbar = SnackBar(
                    content=Text("Error: User ID already exists!", color="White"),
                    bgcolor="red"  # Use a distinct color for errors
                )
                self.page.snack_bar = snackbar
                self.page.snack_bar.open = True
                self.page.update()
                return  # Exit the function since we don't want to proceed

            # Check if the email or username already exists
            c.execute("SELECT email FROM registration WHERE email = ?", (email,))
            if c.fetchone() is not None:
                snackbar = SnackBar(
                    content=Text("Error: Email already exists!", color="White"),
                    bgcolor="red"
                )
                self.page.snack_bar = snackbar
                self.page.snack_bar.open = True
                self.page.update()
                return  # Exit the function since email is not unique

            # Insert data into the registration table (if no errors)
            c.execute('''
                INSERT INTO registration (id, full_name, user_name, email, password, security_qna)
                VALUES (?, ?, ?, ?, ?, ?)
            ''', (userid, full_name, user_name, email, password, security_qna))
            ...
            snackbar = SnackBar(
                content=Text(f"Registration successful for {user_name}!", color="White"),
                bgcolor="green"
            )
            self.page.snack_bar = snackbar
            self.page.snack_bar.open = True
            ...
            # Handle unexpected errors and show a generic error message
            snackbar = SnackBar(
                content=Text(f"Error: {str(e)}", color="White"),
                bgcolor="red"
            )
            self.page.snack_bar = snackbar
            self.page.snack_bar.open = True
    ...
    def clear_fields(self):
        # Properly clear the input fields on the registration page
    ...
    def show_dashboard(self, user_id):
        ...
        # The task Are Displaying
        # self.page.add(task_display)
        ...
        # This 1st button is correctly working :
        BTN = TextButton(text="Log Out", on_click=lambda _: self.show_logout_confirmation())
        # BTN = TextButton(text="Log Out", on_click=lambda _:self.show_logout_confirmation)
        ...
        self.page.add(
            Container(
                content=Column(
                    [Container(height=20), welcome_message, Container(height=20), dashboard_buttons],
                    alignment=MainAxisAlignment.CENTER,
                    horizontal_alignment=CrossAxisAlignment.CENTER,
                    expand=True
                ),
    ...
    def show_logout_confirmation(self):
        ...
        def confirm_logout(e):
            ...
            self.show_login_page()  # Proceed with logout
            self.page.update()

        def cancel_logout(e):
            self.page.dialog.open = False  # Close the dialog without logging out
            self.page.update()

        logout_dialog = AlertDialog(
            title=Text("Confirm Logout"),
            content=Text("Are you sure you want to log out?"),
            actions=[
                ElevatedButton(text="Yes", on_click=confirm_logout, bgcolor="green", color="white"),
                ElevatedButton(text="No", on_click=cancel_logout, bgcolor="red", color="white"),
            ],
    ...
    ## New Function Added:  ( Working Perfectly )
    def show_read_tasks_page(self, user_id):
        ...
        if not user_tasks:
            ...
            #             Text(f"Priority: {task['priority']}", color="white", weight="bold"),
            #             Text(f"Status: {task['status']}", color="white", weight="bold"),
            #             Text(f"Deadline: {task['deadline']}", color="white", weight="bold")
            #         ]),
            #         padding=10,
            ...
            for task in user_tasks:
                ...
                if 'user_id' in task:
                    ...
                    # Create a scrollable container for each task
                    task_container = Container(
                        content=Column([
                            Text(f"Task: {task['name']}", color="white", weight="bold"),
                            Text(f"Description: {task['description']}", color="white", weight="bold"),
                        ...
                        scroll="auto"  # Enable vertical scrolling within the card
                        ),
                        padding=10,
                        border=border.all(1, color=colors.BLACK),
                        border_radius=8,
                        bgcolor=bg_color,
                        width=290,  # Adjust width as needed
                        height=200,  # Adjust height as needed to show part of the card and allow scrolling
                    )

                    task_items.append(task_container)


        # Layout for task cards
        def update_layout(event=None):
            screen_width = self.page.window.width
            self.page.on_resized = update_layout  
...
# ###########################################################################################################
    ...
    def show_update_task_form(self, user_id):
        ...
        def update_task(e):
            ...
            # Update the task in the database without user_id
            try:
                c.execute("""
                    UPDATE tasks
                    SET task_description = ?, task_priority = ?, task_status = ?, deadline = ?
                    WHERE task_name = ?
                """, (new_description, new_priority, new_status, new_deadline, task_name))
                conn.commit()
                ...
                self.page.snack_bar = SnackBar(Text(f"Database Error: {e}"), open=True)
        ...
        new_task_deadline_input = TextField(label="New Task Deadline (YYYY-MM-DD)", width=330)
    ...
    def show_delete_task_form(self, user_id):
        ...
        def fetch_tasks_from_db():
            self.tasks = self.get_tasks_by_user(user_id)
            return self.tasks

        def delete_task(task_id):
            try:
                with connect('task_wizard.db') as conn:
                    cursor = conn.cursor()
                    cursor.execute("DELETE FROM tasks WHERE task_id=?", (task_id,))
                    conn.commit()

                    # SnackBar for successful deletion
                    success_snackbar = SnackBar(Text(f"Task '{task_id}' has been deleted successfully."), open=True)
                    self.page.overlay.append(success_snackbar)
                    self.page.update()

                    self.update_task_list()  # Refresh the task list
            except Exception as e:
                # SnackBar for database error
                error_snackbar = SnackBar(Text(f"Database Error: {e}"), open=True)
        ...
        def on_submit_click(e):
            ...
            task_id = next((task['task_id'] for task in tasks if task['name'] == task_name), None)
            if task_id:
                delete_task(task_id)
            else:
                # SnackBar for task not found
                task_not_found_snackbar = SnackBar(Text(f"No task found with name '{task_name}'"), open=True)
                self.page.overlay.append(task_not_found_snackbar)
                self.page.update()
    ...
    def update_task_list(self):
        tasks = self.get_tasks_by_user(self.current_user_id)
        self.page.controls.clear()
        for task in tasks:
            self.page.add(Text(f"Task: {task['name']} - Deadline: {task['deadline']}"))
        self.page.update()
